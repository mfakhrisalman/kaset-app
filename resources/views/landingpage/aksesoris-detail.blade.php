    ...
    <div class="container">
        <div class="about-content">
            <div class="row animate-box">
                <div class="col-md-4">
                    <img class="img-responsive" src="{{ asset('storage/' . $aksesoris->images) }}" alt="about">
                </div>
                <div class="col-md-8">
                    <div class="desc">
                        <h2>{{ $aksesoris->name }}</h2>
                        <hr>    
                        <h3>Rp {{ number_format($aksesoris->price, 0, ',', '.') }}</h3>

                        <!-- Product quantity controls -->
                        <div class="product-quantity mb-3">
                            <form id="cart-form-{{ $aksesoris->id }}" action="/aksesoris/cart/detail" method="POST" style="display: none;">
                                @csrf
                                <input type="hidden" name="accessory_id" value="{{ $aksesoris->id }}">
                            ...
                            @can('pelanggan')
                            <a href="javascript:void(0)" class="btn btn-primary btn-outline btn-lg" onclick="submitCartForm({{ $aksesoris->id }})">Tambah Ke Keranjang</a>
                            @else
                            <a href="/login" class="btn btn-primary btn-outline btn-lg" onclick="submitCartForm({{ $aksesoris->id }})">Tambah Ke Keranjang</a>
                            @endcan
                            </div>
